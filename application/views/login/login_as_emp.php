...
<script>
    $(document).ready(function(){
        ...
        $('#btn_login').click(function(event){
            ...
            xhr.open('GET','http://localhost:1235/EHCPS/login/authenticate_user/'+username+"/"+password,true);
            xhr.onload = function(){
            if(this.status == 200){
            var result = JSON.parse(xhr.responseText);

            //console.log(result);
            user_id_sesstion = result;
            //console.log(user_id_sesstion);
            if(result == '0'){
                $('#error_username_or_password').show();
                event.preventDefault();
            }else{
               ...
               var key = result;
                ...
                $.ajax({
              url: "<?php echo base_url();?>login/login_as_emp/" + key,
              method: "GET",
              dataType:"text",
              cache:false,
